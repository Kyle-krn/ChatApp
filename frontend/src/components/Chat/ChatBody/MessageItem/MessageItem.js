...
export const MessageItem = ({myMess, username, message, created_at, isMyMessageUp, type}) => {
    return (
        <div className={`${styles.messageItem} ${myMess? styles.myMessage: styles.otherMessage}`}>
            <span className={styles.username}>{username}</span>
            <span className={styles.messageText}>{message}</span>
            <div className={styles.messageStatus}>
                <span className={styles.messageTime}>{new Date(created_at).toLocaleString("ru-RU", {hour: "numeric", minute:"numeric"})}</span>
